  ...
  (request, _sender, sendResponse) => {
    ...
    if (request.action === "getLocalStorageItem") {
      try {
        ...
        sendResponse({error: e instanceof Error ? e.toString() : 'Unknown error'});
      }
...
function insertSignature(composeElement: HTMLElement, signature: string): void {
  ...
  composeElement.insertAdjacentHTML(
    'beforeend',
    `<br><br><div class="SyncSignature" style="margin-top: 10px;">${signature}</div>`
  );
...
function tryInsertSignature(composeElement: HTMLElement): void {
  chrome.storage.sync.get("selectedSignature", (data: {selectedSignature?: SelectedSignature}) => {
...
if (window.location.hostname === 'mail.google.com') {
  ...
  setTimeout(() => {
        ...
        mutation.addedNodes.forEach((node) => {
          if (node instanceof HTMLElement) {
            if (isComposeElement(node)) {
              ...
              const composeFields = node.querySelectorAll('div[aria-label="Message Body"]');
              if (composeFields.length) {
                console.log(`Found ${composeFields.length} compose fields in subtree`);
    ...
    const existingComposeFields = document.querySelectorAll('div[aria-label="Message Body"]');
    if (existingComposeFields.length) {
      console.log(`Found ${existingComposeFields.length} existing compose fields`);
